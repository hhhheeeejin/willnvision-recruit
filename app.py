...
.hero-section::before {
    ...
    font-size: 1.1rem; opacity: 0.35; transform: rotate(15deg);
}

.hero-emoji { font-size: 2.2rem; margin-bottom: 0.3rem; position: relative; z-index: 1; }

.hero-title {
    font-size: 1.4rem; font-weight: 900; margin: 0.2rem 0;
...
.hero-phone {
    ...
    padding: 0.3rem 0.85rem; border-radius: 16px;
...
.section-header {
    ...
    margin: 1.2rem 0 0.6rem; padding-left: 0.5rem;
...
[data-testid="stExpander"] {
    ...
    box-shadow: 0 1px 4px rgba(66, 133, 244, 0.05) !important;
    transition: all 0.2s;
}
...
# ============================================
    ...
    if not centers:
        ...
        if start_address:
            ...
            ROUTE_HEADER = (
                '<div style="background: linear-gradient(135deg, #EFF6FF 0%, #DBEAFE 100%); '
                'padding: 0.9rem; border-radius: 12px; text-align: center; '
                'border: 1px solid rgba(66, 133, 244, 0.15); margin-bottom: 10px;">'
...
"""
                        ...
                        response = client.chat.completions.create(
                            model="gpt-4o-mini",
                            messages=[
                                {"role": "system", "content": "당신은 한국 출퇴근 경로 안내 전문가입니다."},
            ...
            if st.session_state.get('commute_analysis'):
                try:
                    ...
                    def time_card(key, emoji, label, time_text):
                        ...
                        if is_selected:
                            return (
                                '<div style="background: linear-gradient(135deg, #4285F4, #2563EB); '
