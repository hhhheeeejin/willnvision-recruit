...
(function() {
    ...
    // 즉시 + 0.1초 + 0.3초 + 0.6초 후 반복 실행 (확실하게)
    scrollToTop();
...
if st.session_state.active_tab == "chat":
    ...
    def build_system_prompt():
        ...
        return (
            ...
            f"4. 개인정보 수집 금지\n\n"
            f"[출퇴근 시간 질문 처리]\n"
            f"사용자가 '○○에서 ○○센터까지 얼마나 걸려?', '출근시간', '거리' 등을 물으면:\n"
            f"1. 한국 지리 기반으로 대략적인 시간을 안내 (예: 약 30~40분)\n"
            f"2. 4가지 교통수단 중 적합한 것 2~3개로 추천 (자동차/대중교통/자전거/도보)\n"
            f"3. 대중교통의 경우 환승 경로 간단히 안내\n"
            f"4. 답변 마지막에 반드시 다음 안내 추가:\n"
            f"   '💡 정확한 시간은 카카오맵·네이버지도에서 확인하실 수 있어요!\n"
            f"   하단 ⚡기능 선택의 🚇출근거리 탭에서 바로 길찾기 가능합니다.'\n"
            f"5. 답변 형식 예시:\n"
            f"   '강남역에서 윌앤비전 생각공장 당산까지의 출퇴근 안내드릴게요! 🚇\n"
            f"   \n"
            f"   🚇 대중교통 (추천)\n"
            f"   - 약 30~40분\n"
            f"   - 2호선 강남역 → 합정역 환승 → 당산역\n"
            f"   \n"
            f"   🚗 자동차\n"
            f"   - 약 25~35분 (교통상황 따라)\n"
            f"   - 강남대로 → 양화대교\n"
            f"   \n"
            f"   💡 정확한 시간은...'\n"
            f"6. 부정확한 정보면 솔직하게 '대략적인 안내입니다' 명시"
        )
